...
describe("Controller steam-accounts", () => {
    it("add()", async () => {
        await assert.rejects(SteamAccountsController.add(userId, username, password), (err) => {
    ...
    step("login()", async () => {
        ...
        await assert.rejects(SteamAccountsController.login(userId, username), (err) => {
            assert.equal(err.name, "steamidler");
...
describe("Controller steamclient-actions", () => {
    ...
    it("activatef2pgame()", async () => {
        ...
        const steamAccount = await SteamAccountModel.get(userId, username);
        assert.equal(steamAccount.data.games.some((game) => game.gameid === 1797880), true);
    });
    it("cdkeyRedeem()", async () => {
        await assert.rejects(SteamClientActions.cdkeyRedeem(userId, username, "76CPE-E4CYG-5DXDG"), (err) => {
...
describe("Controller steamcommunity-actions", () => {
    ...
    it("changeAvatar()", async () => {
        // get image blob
        const blob = await fetch("https://avatars.steamstatic.com/23e6beb43897c50a8e004af188539b274d06310b_full.jpg").then((res) => res.blob());
        // get
