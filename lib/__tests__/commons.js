...
import { decrypt, encrypt, isAuthError, isBadPassword, isBadSteamGuardCode, isSteamGuardError, mergeGamesArrays, } from "../commons.js";
describe("common functions", () => {
    ...
    step("mergeGamesArrays()", () => {
        ...
        const { merge, difference } = mergeGamesArrays(game1, game2);
        assert.equal(merge.length, 3);
        assert.equal(difference.length, 1);
        assert.equal(merge.some((game) => game.gameid === 3), true);
        assert.equal(difference.some((game) => game.gameid !== 1), true);
    });
});
