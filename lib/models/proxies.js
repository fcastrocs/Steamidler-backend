 ...
 */
export async function getOneProxy() {
    const collection = await getCollection(collectionName);
    ...
    if (!proxies.length)
        throw ERRORS.PROXY_LIMIT_REACHED;
    return proxies[Math.floor(Math.random() * proxies.length)];
}
export async function deleteProxy(proxy) {
    const collection = await getCollection(collectionName);
