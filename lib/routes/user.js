...
router.post("/googleresponse", async (req, res) => {
    ...
    if (!user) {
        res.cookie("tempSession", { userId, avatar: payload.picture, email: payload.email }, { signed: true, httpOnly: true, expires: new Date(Date.now() + 10 * 60000), secure: true });
 ...
 */
async function authenticateUser(res, req, user) {
    // authenticate user
 ...
 * Verify google token
 */
async function verifyToken(credential, clientId) {
