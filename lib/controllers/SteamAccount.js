 ...
 */
async function fullyLogin(loginOptions, proxy) {
...
async function steamWebLogin(loginRes, proxy) {
    ...
    const steamcommunity = new SteamCommunity(steamId, { host: proxy.ip, port: proxy.port, type: 5, userId: process.env.PROXY_USER, password: process.env.PROXY_PASS }, Number(process.env.SOCKET_TIMEOUT), webNonce);
 ...
 */
async function steamcmLogin(loginOptions, proxy, steamcm) {
    // setup socks options
    const socksOptions = {
        proxy: {
            host: proxy.ip,
 ...
 */
async function restoreAccountState(steam, steamAccount) {
 ...
 */
function disconnectHandler(userId, username, steam) {
...
function isCodeNeededError(error) {
    return (error === "AccountLogonDenied" || // need email code
        error === "AccountLoginDeniedNeedTwoFactor" || // need mobile code
        error === "SteamGuardCodeNeeded");
}
function isBadCodeError(error) {
    return (error === "InvalidLoginAuthCode" || // bad email code
