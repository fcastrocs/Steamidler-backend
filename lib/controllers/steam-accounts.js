...
export async function add(userId, username, password, code) {
    ...
    if (steamVerify) {
        ...
        if (!code)
            throw new SteamClientError(steamVerify.authType);
 ...
 */
async function restoreState(steam, steamAccount, userId, username) {
...
function SteamEventListeners(userId, username, steam) {
    steam.on("loginKey", async (loginKey) => {
        ...
        await SteamAccountModel.updateField(userId, username, { auth: steamAccount.auth });
    });
        ...
        operation.attempt(async (currentAttempt) => {
            ...
            try {
                ...
                if (error instanceof SteamClientError) {
                    ...
                    if (currentAttempt > 1 && isAuthError(error.message))
                        return;
                    // Steam is down
